...
//admin login
    ...
    function issuebook($book,$userselect,$days,$getdate,$returnDate){
        ...
        if ($result > 0) {
            ...
            foreach($recordSet->fetchAll() as $row) {
                $issueid=$row['id'];//get id of user
                $issuetype=$row['type'];//get type of user

                // header("location: admin_service_dashboard.php?logid=$logid");
            }
            foreach($recordSetss->fetchAll() as $row) {
                $bookid=$row['id'];//get id of book
                $bookname=$row['bookname'];//name of book

                    $newbookava=$row['bookava']-1;//make avai of book -1 as it has been assigned now
                     $newbookrent=$row['bookrent']+1;//rent+1 as this book has been allocated
            }
